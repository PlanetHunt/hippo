 ...
 * <p>
 * The maneuver is defined by a direction in satelliteframe. The current
 * attitude of the spacecraft, defined by the current spacecraft state, will be
 * used to compute the thrust direction in inertial frame. A typical case for
 * tangential maneuvers is to use a {@link org.orekit.attitudes.LofOffset LOF
 * aligned} attitude provider for state propagation and a velocity increment
...
public class PropulsionSystem extends AbstractParameterizable implements ForceModel {
	...
	private DateDetector startDateDetector;
	...
	public FieldVector3D<DerivativeStructure> accelerationDerivatives(final AbsoluteDate date, final Frame frame,
			...
			final FieldRotation<DerivativeStructure> rotation, final DerivativeStructure mass) throws OrekitException {
		if (firing) {
			...
			return new FieldVector3D<DerivativeStructure>(new DerivativeStructure(parameters, order, 0.0),
					new DerivativeStructure(parameters, order, 0.0), new DerivativeStructure(parameters, order, 0.0));
		}
	}
	...
	public FieldVector3D<DerivativeStructure> accelerationDerivatives(final SpacecraftState s, final String paramName)
		...
		if (firing) {
			...
			if (THRUST.equals(paramName)) {
				...
				// acceleration does not depend on flow rate (only mass decrease
				// does)
	...
	public EventDetector[] getEventsDetectors() {
		return new EventDetector[] { this.startDateDetector, this.endDateDetector };
	}
