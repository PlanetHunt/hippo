            ...
            try {
                ...
                if (visitor.stillAcceptsData() && !entry.isDirectory()) {
                    ...
                    if (ZIP_ARCHIVE_PATTERN.matcher(entry.getName()).matches()) {
                        ...
                        Matcher gzipMatcher = GZIP_FILE_PATTERN.matcher(entryName);
                        String baseName = gzipMatcher.matches() ? gzipMatcher.group(1) : entryName;
                        String baseName2 = baseName;
                        if(baseName2.contains("/"))
                        	baseName2 = baseName2.substring(baseName2.lastIndexOf("/")+1);
                        if(baseName2.contains("\\"))
                        	baseName2 = baseName2.substring(baseName2.lastIndexOf("\\")+1);
                        if (supported.matcher(baseName).matches() || supported.matcher(baseName2).matches()) {
                            // visit the current entry
                            final InputStream stream = gzipMatcher.matches() ? new GZIPInputStream(entry) : entry;
