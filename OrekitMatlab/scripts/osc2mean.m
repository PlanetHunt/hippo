...
a = sin(oetmp(6)) * sqrt(1 - oetmp(2) * oetmp(2));
...
oetmp(6) = mod(eanom - oetmp(2) * sin(eanom), 2.0 * pi);
...
aa = 1/3 - 0.5 * sin(ios)^2;

bb = 0.5 * sin(ios)^2;

if (eos < 0.01)
    lamos = mod(maos + apos, 2.0 * pi);
...
oemean(2) = em;
oemean(3) = im;
oemean(4) = mod(apm, 2.0 * pi);
oemean(5) = mod(ranm, 2.0 * pi);
oemean(6) = mod(mam, 2.0 * pi);

[ea, ta] = kepler1(oemean(6), em);
